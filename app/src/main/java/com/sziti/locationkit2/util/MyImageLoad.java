...
public class MyImageLoad implements ImageLoad {
    ...
    public MyImageLoad(Headers headers) {
    ...
    public void loadImage(final String url, final LoadCallback callback, final ImageView imageView, final String unique) {
        ...
        if (isLocalUri(uri.getScheme())) {
            if (isAssetUri(uri)) {
                ...
                //是本地文件
                Log.e("MyImageLoad", "是本地文件URL的类型");
                loadImageFromLocal(uri.getPath(), unique, imageView);
                return;
            }
        } else {
            if (isNetUri(url)) {
                loadImageFromNet(url, headers, unique, imageView);
                return;
            }
            Log.e("MyImageLoad", "未知的图片URL的类型:" + url);
        }
    }
     ...
     * 从网络加载图片
     */
    private void loadImageFromNet(final String url, final Headers headers, final String unique, final ImageView imageView) {
        OkHttpImageLoad.ImageDownLoadListener loadListener = new OkHttpImageLoad.ImageDownLoadListener() {
            ...
            public void onSuccess() {
                loadImageFromLocal(ImageCacheUtil.getCachedPath(url), unique, imageView);
            }
        ...
        OkHttpImageLoad.load(url, headers, loadListener);
    }
